...
public abstract class AbstractForm extends Composite {
	...
	/**
	 * Use to export field as context.
	 */
	...
	protected AbstractForm(Composite parent, int style, String[] existingNames) {
		super(parent, style);
		this.existingNames = existingNames == null ? new ArrayList<String>() : Arrays.asList(existingNames);
	}
	...
	/**
	 * adds listeners to controls the utils buttons disposed on the form DOC
	 * ocarbone Comment method "addUtilsButtonControls".
	 */
	protected abstract void addUtilsButtonListeners();

	/**
	 * adds controls to parent composite DOC ocarbone Comment method
	 * "addFields".
	 * 
	 * @param form
	 */
	...
	/**
	 * Sets readOnly, adapt the Form to Read Only Mode or edition and execute
	 * checkFieldsValue. DOC ocarbone Comment method setReadOnly.
	 * 
	 * @param readOnly
	 *            the readOnly to set
	 */
			...
			if (isContextMode()) {
				...
				if (contextItem != null) { // create
					...
					ConnectionContextHelper
							.setPropertiesForContextMode(connectionItem, contextItem, getConetxtParams());
	 ...
	 */
	protected void headerRowForSchemaRowNames(FileConnection originalValueConnection,
