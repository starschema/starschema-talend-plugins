...
public abstract class AbstractCreateTableAction extends AbstractCreateAction {
    ...
    protected void createFilePositionalTableWizard(RepositoryNode node, boolean forceReadOnly) {
        ...
        if (node.getType() == ENodeType.REPOSITORY_ELEMENT) {
            ...
            case METADATA_FILE_POSITIONAL:
                ...
                // PTODO OCA : replace getStringIndexed by
                // IndiceHelper.getIndexedLabel(metadataTable.getLabel(),
                // existingNames)
                        ...
                        if (!monitor.isCanceled()) {
                            ...
                            if (!metadataConnection.getDbType().equals(EDatabaseConnTemplate.GODBC.getDBDisplayName())
                                ...
                                String genUrl = DatabaseConnStrUtil.getURLString(metadataConnection.getDbType(),
                                        ...
                                        metadataConnection.getUsername(), metadataConnection.getPassword(), metadataConnection
                                                .getPort(), metadataConnection.getDatabase(), metadataConnection
                                                .getFileFieldName(), metadataConnection.getDataSourceName(), metadataConnection
                            ...
                            if (creation) {
                                ...
                                ConnectionHelper.addPackages(newDataPackage, (DatabaseConnection) metadataConnection
                                        .getCurrentConnection());
                                List<String> itemTableName = null;
                                // modified by nma, open schema editor even failed
                                // to connect
                                ...
                                if (noTableExistInDB) {
                                    MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
                                            Messages.getString("AbstractCreateTableAction.retrieveForbidden"), Messages
                                    ...
                                    UIJob uijob = new UIJob("") { //$NON-NLS-1$
                                        ...
                                        // modified by wzhang. when connection
                                        // failed,error message display.
                                        public IStatus runInUIThread(IProgressMonitor monitor) {
                                            if (!managerConnection.getIsValide()) {
                                                MessageDialog.openError(null, Messages
                                                        .getString("AbstractCreateTableAction.connError"), //$NON-NLS-1$
    ...
    public boolean checkConnectStatus(ManagerConnection managerConnection, IMetadataConnection metadataConnection) {
        ...
        if (managerConnection.getIsValide()) {
            List<String> itemTableName = null;
            // hyWang remove the second parameter of method
            // returnTablesFormConnection for bug7374
