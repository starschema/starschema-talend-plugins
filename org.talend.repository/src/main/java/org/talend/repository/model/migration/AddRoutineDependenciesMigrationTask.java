...
public class AddRoutineDependenciesMigrationTask extends AbstractJobMigrationTask {
    ...
    public ExecutionResult execute(Item item) {
        ...
        try {
            ...
            if (null != item2.getProcess()) {
                ParametersType parameterType = item2.getProcess().getParameters();
                if (parameterType == null) {
                    parameterType = TalendFileFactory.eINSTANCE.createParametersType();
                    item2.getProcess().setParameters(parameterType);
                }
                List<RoutinesParameterType> routinesDependencies = (List<RoutinesParameterType>) parameterType
