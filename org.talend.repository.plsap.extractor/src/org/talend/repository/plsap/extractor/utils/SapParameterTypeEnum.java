...
public enum SapParameterTypeEnum {
	INPUT_SINGLE, INPUT_STRUCTURE, INPUT_TABLE, OUTPUT_SINGLE, OUTPUT_STRUCTURE, OUTPUT_TABLE, TABLE_INPUT, TABLE_OUTPUT;
	...
	public static final String[] getOutputLabels() {
		List<String> paramList = new ArrayList<String>();
		for (SapParameterTypeEnum paramType : values()) {
			if ((!((SapParameterTypeEnum) paramType).getDisplayLabel().toLowerCase().startsWith("output")) && (!((SapParameterTypeEnum) paramType).getDisplayLabel().toLowerCase().endsWith("output"))) {
				continue;
			}
